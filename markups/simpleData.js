function markupSimpleData (data, type, row, settings) {
	...
	if(settings['inProprotionTo']) {
		...
		var markup =
			...
			<span class="badge text-bg-secondary" title="${data[2001].toLocaleString('hu-HU', {maximumFractionDigits: 2})} (2001-es adat)">
				${proptional[2001].toLocaleString('hu-HU', {maximumFractionDigits: 2})}%</span>&nbsp;
			<span class="badge text-bg-secondary" title="${data[2011].toLocaleString('hu-HU', {maximumFractionDigits: 2})} (2011-es adat)">
		...
		var markup =
			`<span class="badge text-bg-primary even-larger-badge" title="2022-es adat">
				${data[2022].toLocaleString('hu-HU', {maximumFractionDigits: 2})}</span><br/>
			...
			<span class="badge text-bg-secondary" title="2011-es adat">
				${data[2011].toLocaleString('hu-HU', {maximumFractionDigits: 2})}
			</span>&nbsp;`						
	}
	return markup
}
